 ...
 * A majority(大多数) of followers must log(记录) the request before
 ...
 * 这里的事务指的是最终的提议Proposal)。
 * （3）Leader：(处于该状态的)服务器将处理请求，并将这些请求转发给其它Follower。
 * 大多数Follower在该请求被批准之前(before it can be accepted)都必须
...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
    ...
    private void loadDataBase() {
		...
		try {
            ...
            // load the epochs
            // 从最新的zxid恢复epoch变量，zxid是64位，前32位是epoch，后32位是zxid
    ...
    synchronized public void startLeaderElection() {
        ...
        // 根据配置获取选举算法
        // 配置选举算法，选举算法3种，可以通过的zoo.cfg里面进行配置，默认是fast选举
    ...
    public void run() {
        ...
        try {
            ...
            while (running) {
                ...
                case LOOKING:
                    LOG.info("LOOKING"); // 如果是LOOKING，则金额选举流程
                    ...
                    if (Boolean.getBoolean("readonlymode.enabled")) {
                        ...
                        try {
                            roZkMgr.start();
                            setBCVote(null);
                            // 此处通过策略模式来决定当前是那个选举算法来进行leader选举
     ...
     * ensemble（全体）.
     * view是一个节点对整个系统成员的当前称呼
     */
    // 该方法返回的是当前集群中所有Server，包含Observer
    // key为sid，value为Server
    ...
    // 从view中将所有observer过滤掉，返回的是所有可以进行选举投票的server
